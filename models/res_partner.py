...
# Default configuration constants - loaded from environment variables
...
class ResPartner(models.Model):
    ...
    def _get_delivery_settings(self):
        """
        Get delivery configuration settings from environment variables.
        ...
        """
        return {
            'origin_lat': float(os.environ.get(
                'DELIVERY_ORIGIN_LATITUDE',
                DEFAULT_ORIGIN_LAT
            )),
            'origin_lon': float(os.environ.get(
                'DELIVERY_ORIGIN_LONGITUDE',
                DEFAULT_ORIGIN_LON
            )),
            'rate_per_mile': float(os.environ.get(
                'DELIVERY_RATE_PER_MILE',
                DEFAULT_RATE_PER_MILE
            )),
            'max_distance': float(os.environ.get(
                'DELIVERY_MAX_DISTANCE',
                DEFAULT_MAX_DELIVERY_DISTANCE
            )),
            'road_multiplier': float(os.environ.get(
                'DELIVERY_ROAD_MULTIPLIER',
            ...
            'use_google_maps': os.environ.get(
                'DELIVERY_USE_GOOGLE_MAPS',
                'false'
            ).lower() in ('true', '1', 'yes'),
        }
